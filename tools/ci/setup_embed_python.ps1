...
$MatrixArch = if ($args.Count -gt 0) { $args[0] } else { "x86_64" }
if ($MatrixArch -eq "x86_64") {
    $Architecture = "amd64"
} elseif ($MatrixArch -eq "aarch64") {
    $Architecture = "arm64"
} else {
    Write-Host "error: unsupported architecture $MatrixArch" -ForegroundColor Red
    exit 1
}
...
try {
    if (-not $PipInstalled) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "setup_pip.py failed with exit code $LASTEXITCODE"
        }
