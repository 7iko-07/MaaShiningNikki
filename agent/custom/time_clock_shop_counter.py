...
@AgentServer.custom_action("time_clock_shop_counter")
class TimeClockShopCounterAction(CustomAction):
    def run(
        self,
        context: Context,
        ...
        if operation == "reset":
            ...
            logger.info("time_clock_shop_counter: reset solo run count")
            return True

        if operation == "start_wait":
            _wait_started_at[task_id] = time.monotonic()
            logger.info("time_clock_shop_counter: start solo result wait timer")
            return True

        if operation != "count":
            logger.error(f"time_clock_shop_counter: invalid operation={operation!r}")
            return False
        ...
        if not 0 <= configured_runs <= 3:
            logger.error(
                "time_clock_shop_counter: max_runs must be between 0 and 3, "
        ...
        if not context.override_next(argv.node_name, next_nodes):
            logger.error(
                "time_clock_shop_counter: failed to set next nodes "
                f"to {next_nodes!r}"
            )
            return False
...
@AgentServer.custom_recognition("time_clock_shop_wait_timeout")
class TimeClockShopWaitTimeoutRecognition(CustomRecognition):
    def analyze(
        self,
        context: Context,
        ...
        if timeout_seconds <= 0:
            logger.error(
                "time_clock_shop_wait_timeout: timeout_seconds must be positive, "
        ...
        if started_at is None:
            ...
            logger.warning(
                "time_clock_shop_wait_timeout: wait timer was not initialized; "
